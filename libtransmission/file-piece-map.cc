...
void tr_file_piece_map::reset(tr_torrent_metainfo const& info)
{
    ...
    std::transform(
        ...
        [](auto const& file) { return file.length(); });
    reset(info.blockInfo(), std::data(file_sizes), std::size(file_sizes));
}
